...
exec python -m kgml_new.scripts.run_gpu_method \
  ...
  --prepared-dataset-cache "${REPO}/cache/primekg-prepared_edge_s42_d128.pkl" \
  --epochs 100 \
  --semantic \
  --strict-semantic \
  ...
  --edge-dim 128 \
  --semantic-cache "${REPO}/cache/primekg-relations-openai.pt" \
  --node-embeddings-path "${REPO}/cache/primekg-node-embeddings-openai.pt" \
  --log-file "${REPO}/logs/slurm/run-kgml-easnemb-primekg-edge-openai-${SLURM_JOB_ID:-local}.log" \
  --log-level INFO \
  --output "${OUT_JSON}"
